...
def determine_type(action_verb: str) -> str:
    ...
    if any(x in action for x in ['debited', 'paid', 'payment', 'withdrawal', 'debit']):
        return "debit"
    if any(x in action for x in ['credited', 'received', 'added', 'credit']):
        return "credit"
    return "debit"
    ...
    for name, config in PATTERNS.items():
        ...
        if match:
            ...
            try:
                ...
                print(f"Regex match found but extraction failed for {name}: {e}")
                return None
    return None
